...
class Asteroid {
public:
	virtual std::vector<std::unique_ptr<Asteroid>> Split() {
		return {};
	}
...
public:
	...
	std::vector<std::unique_ptr<Asteroid>> Split() override {
		std::vector<std::unique_ptr<Asteroid>> result;

		if (render.size & Renderable::LARGE) {
			for (int i = 0; i < 2; ++i) {
				auto a = std::make_unique<TriangleAsteroid>(Renderer::Instance().Width(), Renderer::Instance().Height());
				a->render.size = Renderable::MEDIUM;
				a->transform.position = transform.position;
				result.push_back(std::move(a));
			}
		}
		else if (render.size & Renderable::MEDIUM) {
			for (int i = 0; i < 2; ++i) {
				auto a = std::make_unique<TriangleAsteroid>(Renderer::Instance().Width(), Renderer::Instance().Height());
				a->render.size = Renderable::SMALL;
				a->transform.position = transform.position;
				result.push_back(std::move(a));
			}
		}
...
public:
	...
	std::vector<std::unique_ptr<Asteroid>> Split() override {
		...
		if (render.size & Renderable::LARGE) {
			for (int i = 0; i < 2; ++i) {
				auto a = std::make_unique<SquareAsteroid>(Renderer::Instance().Width(), Renderer::Instance().Height());
				a->render.size = Renderable::MEDIUM;
				a->transform.position = transform.position;
				result.push_back(std::move(a));
			}
...
public:
	...
	std::vector<std::unique_ptr<Asteroid>> Split() override {
		...
		if (render.size & Renderable::LARGE) {
			for (int i = 0; i < 2; ++i) {
				auto a = std::make_unique<PentagonAsteroid>(Renderer::Instance().Width(), Renderer::Instance().Height());
				a->render.size = Renderable::MEDIUM;
				a->transform.position = transform.position;
				result.push_back(std::move(a));
			}
...
enum class WeaponType { TEXTURED, LASER, BULLET, COUNT };
class Projectile {
public:
	Texture2D texture{};
	float scale = 1.0f;
	Projectile(Vector2 pos, Vector2 vel, int dmg, WeaponType wt)
		: transform{ pos }, physics{ vel }, baseDamage{ dmg }, type{ wt }
	{
		if (type == WeaponType::TEXTURED) {
		...
		if (type == WeaponType::TEXTURED) {
			texture = LoadTexture("bullet.png");
			if (texture.id == 0) {
				TraceLog(LOG_ERROR, "Failed to load texture 'bullet.png'");
			}
			scale = 0.15f;
		}
	}
	...
	void Draw() const {
		...
		else if (type == WeaponType::TEXTURED) {
			Vector2 dstPos = {
	...
	float speed)
{
	...
	if (wt == WeaponType::LASER)
		...
		return Projectile(pos, vel, 30, wt);
}
...
public:
	PlayerShip(int w, int h) : Ship(w, h) {
		texture = LoadTexture("spaceship3.png");
...
public:
	...
	void Run() {
		...
		WeaponType currentWeapon = WeaponType::TEXTURED;
		float shotTimer = 0.f;
			...
			for (auto pit = projectiles.begin(); pit != projectiles.end();) {
				...
				for (auto ait = asteroids.begin(); ait != asteroids.end();) {
					...
					if (dist < pit->GetRadius() + (*ait)->GetRadius()) {
						// Pobierz dzieci z rozbicia
						auto newAsteroids = (*ait)->Split();

						// Dodaj dzieci do głównej listy
						for (auto& newA : newAsteroids) {
							asteroids.push_back(std::move(newA));
						}

						// Usuń trafioną asteroidę i pocisk
